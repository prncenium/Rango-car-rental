...
export function PreFooterSection({
  ...
  heading = "Your next car is one admin-approved listing away.",
  body = "Browse what's live right now, or list your own car and let an admin get it in front of real renters.",
  showBrowseCta = true,
  backgroundImage = PRE_FOOTER_IMAGE_URL,
}: {
  eyebrow?: string;
  heading?: string;
  body?: string;
  /** Hide the "Browse available cars" button on pages where that action is redundant (e.g. the Browse page itself). */
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      ...
      >
        ...
        <p className="mt-4 max-w-xl text-body-lg text-neutral-200">{body}</p>
